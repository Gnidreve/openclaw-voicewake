...
Gateway-Audio-Migration 0.2.5/0.2.7), nicht dazwischengeschoben: Die
...
| Version | Thema |
|---|---|
| 0.3.1 | Wake-Word-Schwellwert-Härtung (dazwischengeschoben, kein Testing/Struktur-Thema): Feldtest-Datenpunkte aus `ideas.md` - echte Treffer bei Scores 0.50-0.98, Grundrauschen bei ~0.20, `--threshold 0.5` aktueller Arbeitswert; gelegentliche Fehltrigger bei Husten/"A"-Anlaut-Wörtern (Wake-Word "Alexa"), aktuell ungefährlich da die VAD sie vor OpenClaw/Gateway abfängt. Schwellwert lebt im externen Wake-Word-Listener, nicht in dieser Binary - Umfang hier: klären, ob `wakeword.rs`/das CLI-Adapter-Protokoll die Confidence-Scores des Listeners durchreichen/mitloggen soll, um den Wert pro Raum/Mikrofon einstellbar zu machen, plus ggf. Anpassung des dokumentierten Erfahrungswerts in README/`config.example.toml`. **Update aus dem Live-Feldtest (SSH-Diagnose auf dem Mac mini, 2026-09-16):** Debug-Log des Listeners ausgewertet - fast jeder Prozessneustart endet in einem Trigger, in Bursts korreliert mit realer Raumaktivität (TV/Gespräch), nicht durchgehend. Ursache vermutlich zweiteilig: (a) `wakeword_listener.py` triggert auf einen einzelnen 80ms-Frame ohne Score-Glättung über mehrere Frames; (b) nach einer leeren Runde (kein Transkript) startet der Listener ohne Cooldown sofort neu, was bei anhaltendem Hintergrundgeräusch zu schnellen Trigger-Bursts führt. Ergänzter Scope: (b) ist in dieser Rust-Binary behebbar (kurzer Cooldown nach einer Runde ohne Antwort, bevor der Wake-Word-Listener neu startet) - (a) liegt in `wakeword_listener.py`, außerhalb des Repos, muss direkt auf dem Zielsystem angepasst werden |
| 0.3.2 | Codebase-Refine nach `rust-skills`-Review (dazwischengeschoben, kein Testing/Struktur-Thema): Review von `src/` gegen die neu installierten Skills `coding-guidelines`/`m15-anti-pattern` (siehe `.agents/skills/`) - Ergebnis ist bereits vorab durchgeführt und im Chat dokumentiert. Konkrete Punkte: (1) `main.rs` - `let _ = sm.transition(State::Idle)` durch ein `.expect(...)` mit begründendem Text ersetzen, da diese Transition laut `allowed_next()` von jedem Zustand aus erlaubt ist und daher nie fehlschlagen sollte - ein stiller Fehlschlag würde eine künftig gebrochene Invariante verschlucken statt sie aufzudecken; (2) uneinheitliche Behandlung von Best-Effort-Cleanup-Fehlern vereinheitlichen - manche Aufräum-Löschungen loggen bei Fehlschlag (`if let Err(e) = ... { warn!(...) }`), andere verschlucken still per `let _ =` (`device_identity.rs`, `tts/local.rs`, `transcript_log.rs`, `transcribe/local.rs`, `instance_lock.rs`) - eine Konvention festlegen; (3) fehlender SAFETY-Kommentar beim `unsafe`-Testhelfer in `child_process.rs` (`process_exists`); optional (4) lange Funktionen (>50 Zeilen: `config::validate`, `audio::start_capture`, `transcribe::gateway::transcribe_via_gateway`, `main::record_until_silence`, `main::main`, `wakeword::wait_for_wakeword`, `gateway::connect_and_handshake`) auf Aufteilung prüfen - Geschmacksfrage, kein Bug. Der Review selbst ist mit den installierten Skills jederzeit wiederholbar, falls sich der Code seitdem weiterentwickelt hat |
| 0.3.3 | `tests/`-Struktur vor den neuen Regressionstests festlegen: gemeinsame Test-Helfer (Stub-/Mock-Erzeugung wie in `tests/pipeline_with_stubs.rs`) in ein wiederverwendbares Modul auslagern, statt sie in jedem künftigen Feature-Test erneut zu duplizieren |
| 0.3.4 | Unit-Tests für das Ergebnis der `src/`-Restrukturierung (0.3.0) nachziehen, wo sich durch das Verschieben Lücken ergeben - die fachliche Logik selbst ist bereits aus den 0.2.x-Schritten getestet |
| 0.3.5 | System-Level-Regressionstest in `tests/` für den kompletten Pfad über alle in 0.2.x aktivierbaren Kombinationen (`transport` x `audio_pipeline`) hinweg, gegen ein Mock-Gateway statt echtem OpenClaw (analog zu `tests/pipeline_with_stubs.rs`) |
