...
/// Synthetisiert `text` über das Gateway (`tts.speak`, `audio_pipeline =
/// "gateway"` - Ersatz für den lokalen Piper-Aufruf, siehe `tts.rs`) und
/// spielt das Ergebnis über `tts::play_audio_file` ab.
///
/// Anders als `chat.send` und `talk.session.*` ist `tts.speak` ein
/// einfacher synchroner Request/Response-Aufruf ohne Session-Lifecycle und
/// ohne Events - der Server liefert das fertige Audio (base64, Format laut
/// serverseitig konfiguriertem TTS-Provider - MP3, WAV, Opus, ... nicht
/// fest vorgegeben wie bei der G.711-mu-law-Transkriptionssession) direkt
...
pub async fn synthesize_via_gateway(cfg: &Config, text: &str, tmp_dir: &Path) -> Result<()> {
    ...
    let play_result = crate::tts::play_audio_file(&cfg.tts, &out_path).await;
