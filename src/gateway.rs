//! Gemeinsame Low-Level-WebSocket-Verbindung zum OpenClaw-Gateway: Connect-
//! Handshake (Ed25519-Signatur über `device_identity.rs`), Frame-IO und
//! Kanal-Abonnement. Drei getrennte Nutzer bauen jeweils ihre eigene
//! Verbindung darauf auf (`connect_and_handshake` wird pro Aufruf neu
//! verbunden, keine gemeinsam gehaltene Session):
//!   - `openclaw::websocket` (`transport = "websocket"`: `chat.send` +
//!     `--probe-gateway`)
//!   - `transcribe::gateway` (`audio_pipeline = "gateway"`: `talk.session.*`)
//!   - `tts::gateway` (`audio_pipeline = "gateway"`: `tts.speak`)
//!
//! Frame-Formen und Feldnamen sind gegen den tatsächlichen OpenClaw-Quellcode
//! geprüft (nicht nur gegen die Doku):
...
/// Scopes, die für den read-only Prototyp UND `chat.send` ausreichen (siehe
/// `docs/gateway/clients`, Abschnitt "Bereiche auswählen") - einmal
/// angefordert, damit eine spätere volle Integration keinen erneuten
/// Kopplungs-Vorgang mit anderen Scopes auslöst.
...
/// Events, über die `--probe-gateway` laut Roadmap berichten soll. Alles
/// andere (`tick`, `heartbeat`, `presence`, ...) landet nur auf `debug`-Ebene,
/// damit das Log nicht von reinem Transport-Keepalive überflutet wird.
pub(crate) fn describe_event(event: &str) -> Option<&'static str> {
    match event {
...
pub(crate) struct InboundFrame {
    ...
    #[serde(default)]
    pub(crate) error: Option<GatewayErrorPayload>,
    #[serde(default)]
    pub(crate) event: Option<String>,
}

#[derive(Debug, Deserialize)]
pub(crate) struct GatewayErrorPayload {
...
pub(crate) async fn read_frame(ws: &mut WsStream, timeout_secs: u64) -> Result<InboundFrame> {
...
/// Gemeinsamer erster Schritt für alle drei Gateway-Nutzer oben - die
/// unterscheiden sich erst danach (Abonnieren-und-Zuhören,
/// Abonnieren-und-Senden, oder eine eigene Session-/Request-Form).
pub(crate) async fn connect_and_handshake(cfg: &Config) -> Result<WsStream> {
...
/// Session. Pflicht vor `chat.send`: `chat`-Events werden vom Gateway nur an
/// `sessionKeys`-Topics gesendet, die die Verbindung abonniert hat - ohne
/// dieses Abonnement käme trotz erfolgreichem `chat.send`-ACK nie eine
/// Antwort an. `talk.session.*`/`tts.speak` brauchen das dagegen NICHT: ihre
/// Events/Antworten gehen direkt an die anfragende Verbindung
...
mod tests {
    ...
    fn temp_identity() -> DeviceIdentity {
        let path = std::env::temp_dir().join(format!(
            "openclaw-voicebridge-test-gateway-{}.json",
            uuid::Uuid::new_v4()
        ));
